...
data_cols <- names(agg)[-(1:7)] # first 7 columns aren't data source columns
data_cols <- data_cols[(1:(length(data_cols)-1))] # neither is the last one!
df <- agg %>%
...
nodes <- nodes %>% 
  mutate(group = case_when(
    (name %in% df$data_source) & ! (name %in% df$`Source name`) ~ "raw", 
     TRUE ~ "compilation" 
  ))
