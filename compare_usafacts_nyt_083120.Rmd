...
g <- bind_rows(nyt_total, usafacts_total) %>% select(-deaths) %>% spread(source, cases) %>% 
  ...
  mutate(panel=case_when(source=="difference"~"Difference",
         ...
         color=case_when(source=="NYT"~"NYT",
                         ...
                         T~"Comparison"),
         color=fct_rev(color)) %>%
  ggplot(aes(x=date, y=cases, color=color, text=cases)) + geom_line() + 
...
g <- bind_rows(nyt_total, usafacts_total) %>% select(-cases) %>% spread(source, deaths) %>% 
  ...
  mutate(panel=case_when(source=="difference"~"Difference",
         ...
         color=case_when(source=="NYT"~"NYT",
                         ...
                         T~"Comparison"),
         color=fct_rev(color)) %>%
  ggplot(aes(x=date, y=deaths, color=color, text=deaths)) + geom_line() + geom_hline(yintercept=0, linetype="dotted") +
...
g <- bind_rows(nyt_unknown, usafacts_unknown) %>% filter(cases>0) %>%
  ...
  ggplot(aes(x=State, y=Unallocated, fill=source)) +
...
bind_rows(nyt_GA_un, usafacts_GA_un) %>%
  ggplot(aes(x=date, y=cases, color=source, text=cases)) + geom_line() +
...
g<-case_diff_tab %>% 
  ...
  ggplot(aes(x=NYT, y=USAFacts, label=county, text=county)) + 
...
g<-death_diff_tab %>%
  ...
  xlab("Deaths reported in NYT") + ylab("Deaths reported in USAFacts") +
  theme_mpr() + 
  scale_x_continuous(label=comma, limits=c(0,2750)) +
  scale_y_continuous(label=comma, limits=c(0, 2750)) +
  scale_color_mpr(type = "qual", discrete = TRUE) +
  scale_fill_mpr(type = "qual", discrete = TRUE)
ggplotly(g, tooltip=c("text","x","y"))
```
