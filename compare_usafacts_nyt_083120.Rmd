...
The case counts reported in the two sources are similar, and have gotten more similar in the past couple months. During the month of August, NYT consistantly reported approximately 0.6% more cases than USAFacts, compared to a difference of 1% throughout May and June.
...
g <- bind_rows(nyt_GA_un, usafacts_GA_un) %>%
  ggplot(aes(x=date, y=cases, color=source)) + geom_line() +
  labs(color="") +
  theme_mpr() + 
  scale_color_mpr(type = "qual", discrete = TRUE) +
  scale_fill_mpr(type = "qual", discrete = TRUE)
...
#### Cummulative cases in Georgia over time
```{r}
